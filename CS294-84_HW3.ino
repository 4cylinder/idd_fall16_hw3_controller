...
void loop() {
  ...
  if (softpotReading > SOFTPOT_MIDDLE && softpotCounter==0) { // right side pressed
    Serial.println(5);
    softpotCounter = 1;
  } else if (softpotReading > SOFTPOT_NOT_PRESSED && softpotReading <=SOFTPOT_MIDDLE && softpotCounter==0) { // left side pressed
    Serial.println(4);
    softpotCounter = 1;
